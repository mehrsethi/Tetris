...
//assume box1 is the smaller part of the L (the one on the bottom left in the above diagram),
//box2 the one closer on the longer side (the one to the right of the pivot in the diagram),
//box3 the further on the longer side (the rightmost in the above diagram)
package model;
...
public class RightL extends Block { 
	...
	//constructor
	public RightL(double pivotX1, double pivotY1, double boxSize) {
		super(pivotX1, pivotY1, boxSize);
		this.side = Facing.BOTTOM; //starts facing downwards by default (the orientation depicted in the diagram above)
		this.type = BlockType.RIGHT_L;
	}
	...
	public int[] getCenterPivotCoordinates(int height, int width) {
		if (side==Facing.LEFT) {
			int[] toReturn = {width/2, height/2-30};
			return toReturn;
		}
			...
			int[] toReturn = {width/2-30, height/2-20};
			return toReturn;
		}
	}
}
