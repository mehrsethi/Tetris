...
//assume that box1 is the one on the pivot's bottom, box2 on the right, box3 on the left (as per the diagram)
package model;
...
public class T extends Block { 
	...
	//constructor
	public T(double pivotX1, double pivotY1, double boxSize) {
		super(pivotX1, pivotY1, boxSize);
		this.side = Facing.BOTTOM; //starts by facing bottom as default (the orientation in the above diagram)
	...
	public ArrayList<Double> getBox3Coordinates() {
		if (this.side==Facing.BOTTOM) {
			//box3 was on the left of the pivot, now moves to the bottom of the pivot
			return moveAdjacent(Facing.LEFT, 1);
		}
		else if (this.side==Facing.LEFT) {
			//box3 was on the top of the pivot, now moves to the left of the pivot
			return moveAdjacent(Facing.TOP, 1);
		}
		else if (this.side==Facing.TOP) {
			//box3 was on the right of the pivot, now moves to the top of the pivot
			return moveAdjacent(Facing.RIGHT, 1);
		}
		else { //block is facing right
			//box3 was on the bottom of the pivot, now moves to the right of the pivot
			return moveAdjacent(Facing.BOTTOM, 1);
		}
	}
	...
	public ArrayList<int[][]> getArr() {
		...
		if (side==Facing.TOP) {
			int[][] arr = {{i,j}, {i,j-1}, {i,j+1}, {i-1,j}};
			int[][] arr2 = {{i,j}, {i,j-1}, {i,j+1}};
			int[][] arr3 = {{i, j+1}};
			int[][] arr4 = {{i, j-1}};
			arrList.add(arr);
			arrList.add(arr2);
			...
			int[][] arr = {{i,j}, {i+1,j}, {i-1,j}, {i,j-1}};
			int[][] arr2 = {{i,j-1}, {i+1,j}};
			int[][] arr3 = {{i,j}, {i+1,j}, {i-1,j}};
			int[][] arr4 = {{i, j-1}};
			arrList.add(arr);
			arrList.add(arr2);
			arrList.add(arr3);
	...
	public ArrayList<int[][]> getFourDirectionsArr() {
		int i = (int)(getPivotY1()/getBoxSize());
		int j = (int)(getPivotX1()/getBoxSize());
		int[][] top = {{i,j}, {i,j-1}, {i,j+1}, {i-1,j}}; //top
		int[][] bottom = {{i,j}, {i,j-1}, {i,j+1}, {i+1,j}}; //bottom
		int[][] left = {{i,j}, {i+1,j}, {i-1,j}, {i,j-1}}; //left
		int[][] right = {{i,j}, {i+1,j}, {i-1,j}, {i,j+1}}; //right
		ArrayList<int[][]> arrList = new ArrayList<int[][]>();
		arrList.add(top);
		arrList.add(bottom);
		arrList.add(left);
